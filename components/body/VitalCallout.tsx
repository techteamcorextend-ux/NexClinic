...
const SEVERITY_STYLE: Record<Severity, { chip: string; text: string; label: string }> = {
  normal: { chip: "bg-teal-50 text-teal-700 ring-teal-600/20", text: "text-slate-900", label: "Normal" },
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="truncate text-sm font-bold text-slate-900">{REGION_LABEL[region]}</p>
        </div>
        ...
        <div className="flex shrink-0 items-center gap-1.5">
          ...
          {locked ? (
            <button
              ...
              className="grid h-6 w-6 place-items-center rounded-full text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 focus-visible:outline focus-visible:outline-2 focus-visible:outline-teal-600"
            >
      ...
      {entry?.vitals.length ? (
          ...
          {entry.vitals.map((vital) => {
            ...
            return (
              <div key={vital.id} className="flex items-baseline justify-between gap-3">
                <dt className="truncate text-xs text-slate-500">{vital.name}</dt>
                <dd className={`shrink-0 text-sm font-semibold tabular-nums ${vitalStyle.text}`}>
                  {vital.value}
                  {vital.unit ? (
                    <span className="ml-1 text-[11px] font-medium text-slate-400">{vital.unit}</span>
                  ) : null}
                </dd>
              </div>
            );
        ...
        <p className="mt-3 text-xs text-slate-500">No readings recorded for this region.</p>
      )}

      {entry?.notes?.length ? (
        <ul className="mt-3 space-y-1 border-t border-slate-100 pt-3">
          {entry.notes.map((note) => (
            <li key={note} className="flex gap-2 text-[11px] leading-relaxed text-slate-600">
      ...
      {locked ? (
        <p className="mt-3 text-[10px] text-slate-400">Pinned · press Esc to release</p>
      ) : null}
    </div>
  );
}
